...
class Wall_Updates {
	...
	{                  $uid=$_SESSION['id'];
	//$t="SELECT M.msg_id, M.uid_fk, M.message, M.created, U.username FROM messages M, users U  WHERE M.uid_fk=U.uid and M.uid_fk="."'$uid'"."and M.msg_id="."'$_SESSION['pid']'"."order by M.msg_id desc";
	    $query = mysql_query("SELECT M.msg_id, M.uid_fk, M.message, M.created, U.username FROM messages M, users U  WHERE M.uid_fk=U.uid  and M.msg_id='$pid' order by M.msg_id desc ") or die(mysql_error());
	   ...
	   $ip=$_SERVER['REMOTE_ADDR'];
		...
		if ($comment!=$result['comment']) {
            $query = mysql_query("INSERT INTO `comments` (comment, uid_fk,msg_id_fk,ip,created) VALUES ('$comment', '$uid','$msg_id', '$ip','$time')") or die(mysql_error());
            	$query1 = mysql_query("UPDATE `messages` SET noc = noc+1 where msg_id='$msg_id'") or die(mysql_error());
	...
	{
	                          $uid=$_SESSION['id'];
		    $query = mysql_query("select msg_id_fk FROM `comments` WHERE com_id='$com_id'") or die(mysql_error());
         while($row=mysql_fetch_array($query))
                    $mid=$row['msg_id_fk'];

	    $query = mysql_query("DELETE FROM `comments` WHERE uid_fk='$uid' and com_id='$com_id'") or die(mysql_error());
	          $query = mysql_query("UPDATE messages set noc=noc-1 where msg_id = '$mid'") or die(mysql_error());
        return true;
